...
button.addEventListener('click',function(){
    let LAT=document.getElementById('search_box').value;
    let LON=document.getElementById('search_box1').value
    if(LAT, LON!=""){
        fetch('https://api.weatherbit.io/v2.0/current/airquality?lon='+LAT+'&lat='+LON+'&key=c407fc46a4ff4a0983fbe473ee5aa17d')
            .then(response => response.json())
            .then(data => {
                document.getElementById('destination').textContent=LAT;
                document.getElementById('destination').textContent=LON;
        ...
        document.getElementById('search_box1').value='';
    }
})